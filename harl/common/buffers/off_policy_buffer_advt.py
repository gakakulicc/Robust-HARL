...
class OffPolicyBufferAdvt:
    ...
    def insert(self, data):
        ...
        if self.idx + length <= self.buffer_size:  # no overflow
            ...
            for agent_id in range(self.num_agents):
                ...
                self.adv_actions[agent_id][s:e] = adv_actions[agent_id].copy()
            ...
            for agent_id in range(self.num_agents):
                ...
                self.adv_actions[agent_id][s:e] = adv_actions[agent_id][0:len1].copy()
            ...
            for agent_id in range(self.num_agents):
                ...
                self.adv_actions[agent_id][s:e] = adv_actions[agent_id][len1:length].copy()
                self.valid_transitions[agent_id][s:e] = valid_transitions[agent_id][
                    len1:length
                ].copy()
                if self.act_spaces[agent_id].__class__.__name__ == "Discrete":
    ...
    def sample(self):
        ...
        sp_adv_actions = np.array(
            [self.adv_actions[agent_id][indice] for agent_id in range(self.num_agents)]
        )

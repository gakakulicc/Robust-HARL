...
from harl.common.fgsm_offpolicy import FGSMOffpolicy
...
from harl.common.buffers.off_policy_buffer_advt import OffPolicyBufferAdvt


class OffPolicyHARunnerTraitor:
    """Base runner for off-policy algorithms."""
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        # adv training
        self.adv_epoch = algo_args["model"]["adv_epoch"]
        ...
        if not self.algo_args["render"]["use_render"]:
            ...
            self.buffer = OffPolicyBufferAdvt(
                {**algo_args["train"], **algo_args["model"], **algo_args["algo"]},
                self.envs.share_observation_space[0],
                self.num_agents,
        ...
        if self.state_adversary:
            self.fgsm = FGSMOffpolicy(algo_args["algo"], self.obs_adversary, self.actor, device=self.device)
    ...
    def run(self):
        ...
        if self.algo_args["eval"]["use_eval"]:
            if self.state_adversary:
                self.eval_adv_state(self.algo_args["train"]["warmup_steps"])
                return
        for step in range(1, steps + 1):
            actions = self.get_actions(
                obs, available_actions=available_actions, add_random=True
            )
            adv_actions = self.get_adv_actions(
                obs, available_actions=available_actions, add_random=True
            )
            (
                new_obs,
            ...
            if step % self.algo_args["train"]["train_interval"] == 0:
                ...
                if step % self.victim_interval == 0:
                    if self.args["algo"] == "hasac_traitor":
    ...
    def warmup(self):
        ...
        for _ in range(warmup_steps):
            ...
            adv_actions = self.sample_actions(available_actions)
            (
                new_obs,
                new_share_obs,
                rewards,
    ...
    def insert(self, data):
        (
            ...
            adv_actions,  # (n_agents, n_threads, action_dim)
    ...
    def get_actions(self, obs, available_actions=None, add_random=True):
        ...
        """
        if self.args["algo"] == "hasac" or "hasac_advt":
            actions = []
            for agent_id in range(self.num_agents):
                if (
    ...
    @torch.no_grad()
    def get_adv_actions(self, obs, available_actions=None, add_random=True):
        """Get actions for rollout.
        Args:
            obs: (np.ndarray) input observation, shape is (n_threads, n_agents, dim)
            available_actions: (np.ndarray) denotes which actions are available to agent (if None, all actions available),
        ...
        for agent_id in range(self.num_agents):
            if (
                ...
                adv_actions.append(
                    _t2n(
                        self.actor[agent_id].get_adv_actions(
                            obs[:, agent_id],
                            available_actions[:, agent_id],
                            add_random,
                        )
        ...
        return np.array(adv_actions).transpose(1, 0, 2)
    ...
    def train(self):
        ...
        (
            ...
            sp_adv_actions,  # (n_agents, batch_size, dim)
        ...
        if self.args["algo"] == "hasac" or "hasac_advt":
            ...
            for agent_id in range(self.num_agents):
                next_actions.append(
                    self.actor[agent_id].get_target_actions(sp_next_obs[agent_id], agent_id)
                )
            self.critic.train(
                sp_share_obs,
                sp_actions,
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            if self.args["algo"] == "hasac" or "hasac_advt":
                ...
                for agent_id in agent_order:
                    ...
                    self.actor[agent_id].adv_actor_optimizer.zero_grad()
                    actor_loss.backward()
                    self.actor[agent_id].adv_actor_optimizer.step()
                    self.actor[agent_id].turn_off_grad()
                    # train this agent's alpha
                    if self.algo_args["algo"]["auto_alpha"]:
                        log_prob = (
                ...
                if self.args["algo"] == "had3qn":
                    ...
                    for agent_id in agent_order:
                        self.actor[agent_id].turn_on_grad()
                        # train this agent
                        actions[agent_id] = self.actor[agent_id].get_adv_actions(
                            sp_obs[agent_id], False
                        )
                        actions_t = torch.cat(actions, dim=-1)
                        value_pred = self.critic.get_values(sp_share_obs, actions_t)
    ...
    def eval(self, step):
        ...
        while True:
            ...
            for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
                if "rendezvous" or "pursuit" in self.args["env"]:
                    one_episode_rewards[eval_i].append(eval_infos[eval_i][0]['true_rewards'][0][0])
                else:
    ...
    @torch.no_grad()
    def eval_adv(self, step):
        if self.random_adversary:
            for i in range(self.num_agents):
                self._eval_adv(step, i)
        else:
            self._eval_adv(step, self.agent_adversary)

    @torch.no_grad()
    def _eval_adv(self, step, adv_id):
        """Evaluate the model"""
        eval_episode_rewards = []
        one_episode_rewards = []
        for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
        ...
        while True:
            ...
            adv_obs = eval_transfer_obs
            eval_adv_actions = self.get_actions(
                adv_obs, available_actions=eval_available_actions, add_random=False
            )
            ...
            if obs_offset > 0:
                ...
                eval_transfer_obs[:, :, transfer_adv_len] = 1
            for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
                if "rendezvous" in self.args["env"]:
                    one_episode_rewards[eval_i].append(eval_infos[eval_i][0]['true_rewards'][0][0])
                else:
                    one_episode_rewards[eval_i].append(eval_rewards[eval_i])
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                if "smac" in self.args["env"]:
                    ...
                    print(
                        f"Eval average episode reward is {eval_avg_rew}, adv id is {adv_id}, eval average episode length is {eval_avg_len}.\n"
                    )
                if "smac" in self.args["env"]:
                    self.log_file.write(
                        ",".join(
                    ...
                    self.log_file.write(
                        ",".join(map(str, [step, adv_id, eval_avg_rew, eval_avg_len])) + "\n"
                    )
                self.log_file.flush()
                self.writter.add_scalar(
                    "eval_average_episode_rewards", eval_avg_rew, step
    ...
    @torch.no_grad()
    def eval_adv_state(self, step):
        if self.state_adversary_all:
            self._eval_adv_state_all(step)
        elif self.random_adversary:
            for i in range(self.num_agents):
                self._eval_adv_state(step, i)
        else:
            self._eval_adv_state(step, self.agent_adversary)

    @torch.no_grad()
    def _eval_adv_state(self, step, adv_id):
        """Evaluate the model"""
        eval_episode_rewards = []
        one_episode_rewards = []
        for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
        ...
        while True:
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                if "smac" in self.args["env"]:
                    ...
                    self.log_file.write(
                        ",".join(map(str, [adv_id, eval_avg_rew, eval_avg_len])) + "\n"
                    )
                self.log_file.flush()
                self.writter.add_scalar(
                ...
                self.writter.add_scalar(
                    "eval_average_episode_length", eval_avg_len
                )
                break
            
    @torch.no_grad()
    def _eval_adv_state_all(self, step):
        """Evaluate the model"""
        eval_episode_rewards = []
        one_episode_rewards = []
        for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
        ...
        while True:
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                if "smac" in self.args["env"]:
                    ...
                    print(
                        f"Eval average episode reward is {eval_avg_rew}, adv agents are all of them, eval average episode length is {eval_avg_len}.\n"
                    )
                if "smac" in self.args["env"]:
                    self.log_file.write(
                        ",".join(
                    ...
                    self.log_file.write(
                        ",".join(map(str, ['all', eval_avg_rew, eval_avg_len])) + "\n"
                    )
                self.log_file.flush()
                self.writter.add_scalar(
                    "eval_average_episode_rewards", eval_avg_rew
    ...
    def restore(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.load_adv_actor:
                self.actor[agent_id].adv_restore(self.algo_args["train"]["adv_model_dir"], agent_id)

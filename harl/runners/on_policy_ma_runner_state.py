...
from harl.common.buffers.on_policy_actor_buffer_state import OnPolicyActorBufferState
...
class OnPolicyMARunnerState:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            self.actor_buffer = []
            for agent_id in range(self.num_agents):
                ac_bu = OnPolicyActorBufferState(
    ...
    def collect(self, step):
        ...
        obs_noise_collector = []
        obs_noise_log_prob_collector = []
        action_collector = []
        action_log_prob_collector = []
        rnn_state_collector = []
        for agent_id in range(self.num_agents):
            obs_noise, obs_noise_log_prob, action, action_log_prob, rnn_state = self.actor[agent_id].get_actions(
            ...
            obs_noise_collector.append(_t2n(obs_noise))
            obs_noise_log_prob_collector.append(_t2n(obs_noise_log_prob))
            action_collector.append(_t2n(action))
            action_log_prob_collector.append(_t2n(action_log_prob))
            rnn_state_collector.append(_t2n(rnn_state))
        # (n_agents, n_threads, dim) -> (n_threads, n_agents, dim)
        obs_noises = np.array(obs_noise_collector).transpose(1, 0, 2)
        obs_noise_log_probs = np.array(obs_noise_log_prob_collector).transpose(1, 0, 2)
        ...
        return values, obs_noises, obs_noise_log_probs, actions, action_log_probs, rnn_states, rnn_states_critic

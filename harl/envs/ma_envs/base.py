...
class World(object):
    def __init__(self, world_size, world_noise, torus, agent_dynamic):
        self.nr_agents = None
        # world is square
    ...
    def step(self):
        ...
        for i, agent in enumerate(self.scripted_agents):
            ...
            if agent.dynamics == 'direct':               
                ...
                # next_coord = np.array(truncated_normal((1,), mean=next_coord, std=0.1, threshold=2.0)).squeeze(axis=0)
                next_coord = np.array(truncated_normal((1,), mean=next_coord, std=noise, threshold=2.0)).squeeze(axis=0)
        ...
        if self.agent_dynamic == 'direct_acc':
            ...
            for i in range(self.action_repeat):
                ...
                if self.torus:
                    ...
                    next_coord = np.where(next_coord < 0, 0, next_coord)
                    next_coord = np.where(next_coord > self.world_size, self.world_size, next_coord)
                # next_coord = np.array(truncated_normal((self.nr_agents,), mean=next_coord, std=0.5, threshold=2.0))
                next_coord = np.array(truncated_normal((self.nr_agents,), mean=next_coord, std=noise, threshold=2.0))

...
class RendezvousEnv(gym.Env, EzPickle):
    ...
    def __init__(self,
                 ...
                 torus=False,
                 dynamics='unicycle'):
        EzPickle.__init__(self, nr_agents, obs_mode, comm_radius, world_size, world_noise, distance_bins, bearing_bins, torus,
                          dynamics)
        self.noise_std = noise_std  # noise level
        self.nr_agents = nr_agents
        self.world_size = world_size
        self.noise = world_noise
        self.obs_mode = obs_mode
        self.world = base.World(world_size, world_noise, torus, dynamics)
    ...
    def step(self, actions):
        ...
        true_rewards = self.get_reward(actions)
        # rewards = truncated_normal((self.nr_agents,), mean=true_rewards, std=self.noise_std, threshold=2.0)
        rewards = truncated_normal((self.nr_agents,), mean=true_rewards, std=0.0, threshold=2.0)
        # done = [self.is_terminal]*self.nr_agents
        done = self.is_terminal
        info = {'true_rewards': true_rewards, 'state': self.world.agent_states, 'actions': actions, 'action_penalty': 0.05 * np.mean(actions ** 2),
                'velocities': np.vstack([agent.state.p_vel for agent in self.agents]),
